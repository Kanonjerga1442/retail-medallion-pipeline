...
This document reproduces a one-row snapshot-diff CDC validation. Run the commands from the repository root in PowerShell. A successful full refresh must exist before this procedure so `data/cdc/state/retail_snapshot` is a complete baseline.
...
Validated Airflow evidence from the incremental update run:
...
The source was then restored in a second incremental run. It detected the reverse change as one `UPDATE`, processed only `2009-12-01`, skipped batches 2 through 8, and completed successfully in 855.75 seconds. The restored source SHA-256 matched the pre-validation backup:
...
```powershell
New-Item -ItemType Directory -Force data/validation_backups | Out-Null
Copy-Item -LiteralPath data/landing_csv/online_retail_II.csv -Destination data/validation_backups/online_retail_II.before_incremental_validation.csv
...
$newValue = '489434,85048,15CM CHRISTMAS GLASS BALL 20 LIGHTS CDC_VALIDATION_UPDATE,12,2009-12-01 07:45:00,6.95,13085.0,United Kingdom'
...
```powershell
$runId = 'cdc_incremental_update_' + (Get-Date -Format yyyyMMdd_HHmmss)
...
Expected description: `15CM CHRISTMAS GLASS BALL 20 LIGHTS CDC_VALIDATION_UPDATE`.
...
$sourcePath = (Resolve-Path data/landing_csv/online_retail_II.csv).Path
$encoding = [System.Text.Encoding]::Latin1
$changedValue = '489434,85048,15CM CHRISTMAS GLASS BALL 20 LIGHTS CDC_VALIDATION_UPDATE,12,2009-12-01 07:45:00,6.95,13085.0,United Kingdom'
...
Get-FileHash -Algorithm SHA256 data/validation_backups/online_retail_II.before_incremental_validation.csv

$restoreRunId = 'cdc_incremental_restore_' + (Get-Date -Format yyyyMMdd_HHmmss)
$airflowConf = "{`"mode`":`"incremental`"}"
...
```powershell
Remove-Item -LiteralPath data/validation_backups/online_retail_II.before_incremental_validation.csv
```
